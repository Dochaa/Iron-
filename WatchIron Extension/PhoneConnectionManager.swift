...
class PhoneConnectionManager: NSObject, ObservableObject {
    ...
    private func process(_ samples: [HKQuantitySample], type: HKQuantityTypeIdentifier) {
        ...
        for sample in samples {
            ...
            session.sendMessage(["heartRate": lastHeartRate], replyHandler: nil)
        }
    }
...
extension PhoneConnectionManager {
    private func receive(message: [String : Any]) {
        session.sendMessage(["String" : "test"], replyHandler: nil)
    ...
    private func handleStartWorkoutSessionMessage(message: [String : Any]) {
        guard let start2 = message[PayloadKey.Arg.start] as? Date else {
        ...
        start()
        WorkoutSessionManagerStore.shared.startWorkoutSession(start: start2, uuid: uuid)
    }
