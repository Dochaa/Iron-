...
class HealthManager {
    ...
    func observerHeartRateSamples(Completion: @escaping ((WorkOutOutPut) -> Void)) {
           ...
           observerQuery = HKObserverQuery(sampleType: heartRateSampleType, predicate: nil) { [unowned self] (_, _, error) in
               ...
               self.fetchLatestHeartRateSample { (sample) in
                   ...
                   DispatchQueue.main.async {
                       ...
                       Completion(WorkOutOutPut(HeartRate: heartRate))
                   }
               }
           }
...
extension HealthManager {
    // completion is called on a background queue!
     func requestPermissions(toShare typesToShare: Set<HKSampleType>?, read typesToRead: Set<HKObjectType>?, completion: @escaping () -> Void) {
