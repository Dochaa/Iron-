...
class GeneralTests: XCTestCase {
    ...
    func testEverkineticVideosExist() {
        let exercises = loadExercises()
        let fileManager = FileManager.default
        
        for exercise in exercises {
            for video in exercise.video {
                let url = ExerciseStore.defaultBuiltInExercisesResourceURL.appendingPathComponent(video)
                XCTAssertTrue(fileManager.fileExists(atPath: url.path), "Video file \(video) does not exist")
            }
        }
    }
